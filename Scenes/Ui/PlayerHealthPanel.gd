...
func _ready():
	GameState.player_health.changed.connect(__animate_loose_health)
...
func __animate_loose_health() -> void:
	var target: CanvasItem = container.get_child(GameState.rules.player_max_health - GameState.player_health.value - 1)
	"""
